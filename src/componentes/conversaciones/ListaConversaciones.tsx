...
export function ListaConversaciones({
  ...
  const conversacionesFiltradas = useMemo(() => {
    const filtradas = CONVERSACIONES_MOCK.filter((c) => {
  ...
  const totalNoLeidos = CONVERSACIONES_MOCK.filter((c) =>
  ...
  return (
    <div className="w-80 bg-white border-r border-slate-200 flex flex-col h-full">
      ...
      <div className="p-4 border-b border-slate-100">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1">
            <button className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors" title="Filtrar notificaciones">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {conversacion.ultimoMensaje && (
                              <span className="text-[9px] text-slate-400 flex-shrink-0 ml-2">
                                {formatTime(conversacion.ultimoMensaje.creadoEn)}
                              </span>
                            )}
                          </div>
                          {conversacion.ultimoMensaje && (
                            <p className="text-[10px] text-slate-400 truncate mt-0.5">
                              <span className="font-medium text-slate-500">
                                {conversacion.ultimoMensaje.remitenteNombre.split(" ")[0]}:
                              </span>{" "}
      ...
      {/* Footer - Estado del usuario */}
      <div className="p-3 border-t border-slate-100 bg-slate-50/50">
        <div className="flex items-center gap-3">
          <Avatar nombre="Super Admin" id={usuarioActualId} size="sm" />
          <div className="flex-1 min-w-0">
