...
import { GET, POST } from "@/app/api/triggers/[id]/historial/route";
...
const context = { params: Promise.resolve({ id: "trigger-ajeno" }) };

describe("historial de triggers sin sesión", () => {
  beforeEach(() => {
    from.mockReset();
  ...
  it.each([
    [
      ...
      () =>
        GET(
          new NextRequest(
            "http://localhost/api/triggers/trigger-ajeno/historial"
          ),
          context
        ),
    ],
